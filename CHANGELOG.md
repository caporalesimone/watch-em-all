...
- **Deleting one product asks in a way you can read at a glance.** The confirmation was four paragraphs of prose under a heading that put a long product name in the middle of the question. Now the product is the heading, its picture sits beside the consequences in a fixed square, and the consequences are three marked lines: ⚠️ the carts it will leave, ⚠️ that the next scrape brings it back and which category to remove from which scraper if you want it gone for good, ℹ️ that its price history is kept. The advice appears only when a **category** delivers the product — a group you set up once, which is the part you would not think of — and it names all of them when more than one does, since removing only one would not stop the product coming back. For a product you watch on its own it is left out: it would be telling you to delete the very thing you are deleting. Pressing **Esc** closes the dialog, exactly as Cancel does.
- **A category has a picture now.** In the list of things you watch, a category row showed an empty grey square where products show their photo — no product lives at a listing's URL, so there was no image to show. It now shows a category placeholder, and a plugin can ship its own images for its page rather than only an icon.
...
_Under the hood:_ **the watch row is the job** — `POST /watches` commits and answers in milliseconds, the scrape follows, drained per scraper under the same run lock a scheduled run takes. Before, the wait sat inside the request and a reload lost the spinner but not the scrape, so people added the same URL twice; the duplicate check was a `SELECT` two minutes before its `INSERT` and is now `UNIQUE (user_id, url)`. Progress and cooperative cancellation live in the **core's** book of in-flight jobs (`plugin_jobs`, CTX-R13) rather than on that row: publishing progress means committing while the work runs, and on a scheduled run the session a plugin holds is the *worker's* — committing it made a half-filled `scrape_user_log` durable, and a crash left it there for ever with a NULL status. **A delivery knows whether it is complete**: a page that could not be read downgrades the run to the non-delisting write path (CATSVC-R2b), because "we could not read page 7" is not "those products are gone" — the confusion that emptied a catalogue in 0.8.1. A category stays **one row** with counters on it, and which inputs delivered a product is recorded separately and many-to-many (`product_sources`), because a product can arrive from two categories at once. **Price history is keyed on the product** — `(plugin_id, external_id)`, no foreign key into the per-user catalog: keyed per row it was one chain per watcher of the same public fact, duplicated and free to diverge, since each entry is written against the previous entry of its own chain. Its counters stay per user (CATSVC-R4b), because asked of the shared chain they would answer for whoever was delivered first. **`ScraperPlugin.build_product`** is `final` and owns what is identical across every scraper (SCR-R18): that assembly had been hand-written in three places across two plugins and had already drifted, `tp_scraper`'s copy carrying the `scraped_at=now()` PROD-R8 forbids. **A mismatched database is legible now**: the drift guard also catches the mirror-image case a *new* version produces — a column the database still requires and the model no longer writes — and the web serves one incompatibility page in place of every route while it lasts (INC-R1..R6), instead of scattering 500s from whichever page touched the wrong table first. Smaller, and still load-bearing: the URL grammar stays in the plugin behind `GET /classify` instead of being copied into TypeScript; the dented filter reads the sanitiser's tag, anchored to the ends of the title, never searching a name the sanitiser has already stripped; the delisting alert works because the baseline keeps delisted members with a `removed` flag, where excluding them left the transition nothing to be a transition from. Three defects surfaced on the way: SQLite ignores `ON DELETE CASCADE` unless `PRAGMA foreign_keys` is on, so the test database disagreed with production about deletion (switching it on exposed six tests writing orphan rows); `scrape_run.products_excluded` had existed since phase 4 with nothing ever writing it, so a run over a category of 39 reported 38 and left the missing one unexplained; and a category had no name at all, because no delivered product lives at the watch's own URL — it now takes the site's breadcrumb. Finally the suite runs across cores (`pytest-xdist`, `-n auto`), 3m41s down to ~25s and no longer the CI's entire critical path; the test client moved to `httpx2` ([#36](https://github.com/caporalesimone/watch-em-all/issues/36)), where Starlette 1.3 looks for it, and the workflow actions to their current majors, in `publish.yml` as well as CI.
