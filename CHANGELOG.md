...
- **In-app alerts.** Watch 'Em All now tells you when something worth buying happens. On each **cart** you pick which changes matter — a product goes **on sale** / its discount ends / goes **out of stock** / is **back in stock**, the **whole cart goes on sale**, or the **savings threshold is reached**. In your **Profile** you set the **cadence**: the weekdays and the time you want to be told. At that time the system compares each cart against the last time it looked and, if anything changed, drops a single **digest** into a new **Alerts** section — what changed, old → new price, where it's from, and the cart's totals and threshold. The sidebar shows an **unread** badge; opening a notification marks it read. It's all **in-app** for now (email and other channels come later), and every notification is always kept in the history. Enabling alerts or changing your cadence **restarts monitoring from now**, so you're told about future changes, not past ones — the app says so where it matters.
