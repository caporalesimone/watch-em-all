...
- **The notification said a price had fallen when it had risen.** An email digest reported `Was €39.92 · Now €49.90 · Discount -0%` on a product that had just come *off* a sale and gone up in price. The column was showing the discount against the product's list price — a different quantity from the two columns beside it, and legitimately zero here — with a minus sign written into the template. It is now a **Difference** column: the signed change between the two prices in the row, `+25%` in red when the price rises, negative in green when it falls, an em dash when there is no earlier price to compare against. ([#37](https://github.com/caporalesimone/watch-em-all/issues/37))
