...
- **Scheduled scraping.** An admin sets per-scraper daily times and the worker runs each scraper automatically at those times — **one scraper at a time**, catching up the most recent missed slot after downtime — so your catalog refreshes on its own, no need to hit *Scrape now*. (Schedules are set via the API for now; the slot editor and the run/log views are the next MVPs.)
- **Schema-drift safety net** (dev): at startup the app compares the database with the code's data model and, when a table or column the code expects is missing from the DB — the situation a model change creates on an existing dev database (there are no migrations yet) — it logs a clear warning, and — with the alert on — surfaces it in an **admin-only** errors feed (`GET /api/admin/errors`) and shows a **banner to the admin** (beside the sidebar, with a Copy-Message button). No more silent 500s after a model change.
...
- Admins get a simple **Plugins** page listing each installed plugin with its **type and version** (e.g. Dragon Store `0.2.0`) — the first slice of admin plugin visibility.
- Admins also get a **Feature flags** page that builds itself from the API — each dev toggle (the worker tick, the scrape-now cooldown) gets an input inferred from its value, so new flags appear with no UI work. The flags are dev-only and reset when the web restarts.
...
_Under the hood:_ the drift check (`src/core/schema_drift.py`) runs after the schema is ensured and iterates the core `Base.metadata` plus each plugin's declared `table_metadata`; it always logs, while `WEA_SCHEMA_DRIFT_ALERT` only gates its appearance in the feed (default off; the env files ship it on). Admin errors are admin-only — schema drift is the first source of a generic `GET /api/admin/errors` feed (`{source, type, title, description}`), never on the public `/api/health` probe; the banner stacks one bar per entry. Plugins that own tables now declare their schema via a `table_metadata` attribute, **enforced at load** by the registry (DB-R7) — which is what lets the drift check see plugin tables. pgweb is dev-only and always-on (no Compose profile); Adminer and the `dev` profile are gone, the gate moving from a profile to file separation (`compose-dev.yml` vs `compose.yml`, INF-3). All app-managed env vars now carry the `WEA_` prefix (external `POSTGRES_*`/`TZ` keep the names their images expect); `pyproject`/`package.json` stay inert placeholders (the product version is baked from the git tag). The `worker` container now runs the real dispatcher skeleton (`src/worker`): it boots like the web (engine, schema, plugins) and writes a per-minute heartbeat, replacing the phase-0 stub. It now **schedules**: a `scraper_schedule` table + admin API (`/api/admin/scrapers`), a TZ-aware due-slot/catch-up calculation, a **serial runner** (one scraper at a time, a per-scraper advisory lock shared with scrape-now so a manual run gets a 409 while one is in progress, a run timeout from `system_settings`), and `scrape_run`/`scrape_user_log` records with counters. The slot editor and the log views are the next (frontend) MVPs. A dev-only feature-flags facility (`feature_flags` table; admin `GET`/`PATCH /api/admin/feature-flags` via Swagger) lets an admin override runtime knobs without a restart — `worker_tick.seconds` (the worker's tick interval) and `scrape_now_cooldown.seconds` (the manual scrape-now cooldown; lower it to e.g. 30s to exercise the UI countdown without waiting the hour), shared with the worker through the DB and reset at web startup (non-persistent). A system log (`system_log`) now captures worker and scraper events through a logging handler (the web's own logs stay on stdout), readable by the admin via `GET /api/admin/logs` (cursor by id — latest N, then only newer rows — with level/source filters); the worker prunes logs and run records past `log_retention_days` (default 90) once a day (MNT-R2), while the price history is never pruned. A transparent scrape cache (`scrape_cache`, CTX-R9) sits inside the context's HTTP client: a repeated `GET` within the per-plugin half-life (default 60 min) is served from the cache — no site visit, no politeness wait — and counted in `cache_hits` (per user and per run), so two users watching the same page in one run cost a single request; it lives behind a small swappable interface (`src/core/scrape_cache.py`). The worker drops a scraper's expired cache at the start of each run (POOL-R3), and an admin can clear a scraper's cache on demand (`DELETE /api/admin/scrapers/{id}/cache`, returns how many entries were removed); the per-plugin half-life setting and the *Clear cache* button land later.
