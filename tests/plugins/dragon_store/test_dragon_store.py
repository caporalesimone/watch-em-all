...
def test_a_cancelled_job_records_no_scan(client: TestClient) -> None:
    ...
    `last_scanned_at` is not a neutral timestamp — it is the switch the list reads to decide
    between "38 products, 1 dented left out Â· read at 14:32" and "never scanned" — and the two
    ...
    really spent are still recorded — in the job's progress, and in the run log.
    """
    import sys
...
class CategoryServer:
    ...
    body goes out. It exists so a test can make something happen **while** a walk is running —
