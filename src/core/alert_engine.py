...
class TextMessageEvent(BaseModel):
    """A flat text notification: title + Markdown body (alert-event.md, AEV-R6/R7). Phase 10.

    The single payload behind **both** text families — ``admin_message`` (written by an admin)
    and ``system_message`` (generated by the core). They differ only by ``kind``, and the
    category a user sees is derived from it rather than stored: two fields that must agree are
    two fields that can disagree.

    No cart structure at all. A notifier tells this apart from a digest by ``kind`` and renders
    ``body`` through the core Markdown helpers — never with parsing of its own (AEV-R7).
    """

    kind: NotificationKind = NotificationKind.ADMIN_MESSAGE
    user_id: int  # the recipient of THIS delivery, not the author
    generated_at: datetime
    title: str
    body: str  # Markdown (CommonMark subset)


# What travels on the notification channel. A notifier receives one of these and switches on
# ``kind``; the union is the honest type for `send`, which phase 7 could still write as "the
# digest" because the digest was all there was.
NotificationEvent = AlertEvent | TextMessageEvent
