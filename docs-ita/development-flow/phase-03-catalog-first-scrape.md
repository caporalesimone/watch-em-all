...
- [x] **3.B12 — `product_properties`, `brand`, `category` + title sanitizer** (~2h): estende il contratto `Product` con **`brand`** (`BrandRef{text, link?}`, [PROD-R6](../4-capabilities/contracts/product.md)), **`product_properties`** (tag, PROD-R5) e **`category`** (breadcrumb `CategoryRef{text, link?}`, PROD-R7), persistiti dal Catalog Update Service (CATSVC-R5); la base `ScraperPlugin` fornisce `add_property()`/`get_properties()` ([SCR-R16](../3-features/plugins/scraper-plugin.md)) e `add_child()`/`get_path()` (SCR-R17), per-prodotto; **sanitizer specifico di Dragon Store** (JSON di etichette hardcoded) toglie le etichette dal titolo → tag, + tag `"Pre Order"` da `PreOrder`; **categoria** dal JSON-LD `BreadcrumbList`. *Verifica: titolo "OFFERTA RAVEN PRIME - X" → name "X" + properties ["Offerta Raven Prime"]; preorder → "Pre Order"; brand con link; category breadcrumb non vuoto.*
...
- [x] **3.F1 — Tabella del Product Picker** (~1h): foto (hover-zoom), titolo+marca+tag+categoria, prezzi, sconto, provenienza (link allo scraper); foto/titolo come link al prodotto (niente colonna "Apri") ([catalog-and-product-picker](../3-features/user/catalog-and-product-picker.md)). *Verifica: tabella popolata e leggibile; categoria e tag visibili.*
