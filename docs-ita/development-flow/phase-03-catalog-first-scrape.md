...
- [ ] **3.B6 — Dragon Store: parsing scheda prodotto** (~1h): fixture salvate; **JSON-LD `Product` come fonte primaria** + DOM per il listino (`tr.D1`), decode `cp1252` + `html.unescape`, tutto **scoped alla tabella principale** (la pagina ha 20-46 prodotti correlati) — vedi [studio ad hoc](../implemented-plugins/dragon-store/capabilities.md#scheda-prodotto-gp-parsing-reale-studio-ad-hoc-giugno-2026); titolo, prezzi, immagine, marca. *Verifica: test verdi sulle fixture.*
- [ ] **3.B7 — Dragon Store: identità + disponibilità** (~1h): `external_id` = id numerico nativo (via `identity_seed`); `is_available` da `offers.availability` a **3 stati** — `InStock`→true, `OutOfStock`→false, `PreOrder`→true (+ tag `"Pre Order"`), sconosciuto→false+log. *Verifica: stessa pagina due volte → stesso external_id; i 3 stati mappati sulle fixture.*
...
- [ ] **3.B12 — Title sanitizer + `product_properties` + `brand`** (~1.5h): estende il contratto `Product` con **`brand`** (`BrandRef{text, link?}`, [PROD-R6](../4-capabilities/contracts/product.md)) e **`product_properties`** (tag, PROD-R5), persistiti dal Catalog Update Service (CATSVC-R5); la base `ScraperPlugin` fornisce `add_property()`/`get_properties()` ([SCR-R16](../3-features/plugins/scraper-plugin.md), per-prodotto); **sanitizer specifico di Dragon Store** guidato da un **JSON di etichette hardcoded nel plugin** che toglie le etichette dal titolo e le aggiunge ai tag, + tag `"Pre Order"` dallo stato `PreOrder`. *Verifica: titolo "OFFERTA RAVEN PRIME - X" → name "X" + properties ["Offerta Raven Prime"]; preorder → property "Pre Order"; brand con link reso assoluto.*

### Frontend

- [x] **3.F1 — Tabella del Product Picker** (~1h): foto, titolo, prezzi, sconto, provenienza, link ([catalog-and-product-picker](../3-features/user/catalog-and-product-picker.md)). *Verifica: tabella popolata e leggibile.*
