...
- [x] **0.T5 — Compose di sviluppo** (~1h): `compose-dev.yml` con `db`+`web`+`worker` (+`adminer` profilo `dev`, `ops` profilo `ops`), healthcheck e log rotation (INF-2) ([deployment](../infrastructure/deployment.md)). *Verifica: `compose up` → tutti healthy, segnaposto nel browser.*
