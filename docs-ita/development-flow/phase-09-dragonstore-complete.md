...
- [ ] **9.X1 — Landing dell'area Admin** (~30m): oggi **non esiste una pagina `/admin`** (nessun `+page.svelte` in `src/frontend/src/routes/admin/`), e la prima voce del gruppo Admin nella sidebar è `/admin/logs` ([`Sidebar.svelte:30`](../../src/frontend/src/lib/components/Sidebar.svelte)) — quindi di fatto l'area si apre sui log di sistema. **Da decidere con Simone**: se i log *devono* essere la landing, si rende esplicito con un redirect `/admin → /admin/logs` invece di dipendere dall'ordine delle voci; se invece la landing giusta è un'altra (una panoramica, o gli scraper), si sposta. Il difetto in entrambi i casi è che il comportamento attuale è un effetto collaterale dell'ordinamento, non una scelta. *Verifica: `/admin` porta dove previsto, e cambiare l'ordine della sidebar non cambia la landing.*
