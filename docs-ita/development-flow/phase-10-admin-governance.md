...
- [ ] **10.B8 — Retention + purge alert** (~1h): retention automatica di log e run nella manutenzione giornaliera ([system-logs-and-maintenance](../3-features/admin/system-logs-and-maintenance.md)); `DELETE /api/admin/alerts?before=`. *Verifica: vecchi oltre retention spariti; notifiche recenti intatte.*
