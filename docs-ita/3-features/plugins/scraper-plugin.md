...
| Tag del prodotto (`tags`) | ✅ (popola) | persiste (non interpreta) |
...
### Tag e categoria
- **SCR-R16** — La base scraper fornisce il **meccanismo** per attaccare a un prodotto una lista di **tag** (`tags`, [product](../../4-capabilities/contracts/product.md) PROD-R5): due metodi `add_tag(value)` (aggiunge una stringa, già **trimmata** e **deduplicata**) e `get_tags()` (restituisce la lista). Operano sul **prodotto in costruzione**, non come stato dell'istanza del plugin (che è un **singleton condiviso**): i tag di un prodotto/utente non devono mai sbordare su un altro. **Cosa** mettere nei tag è scelta del plugin (un'etichetta ripulita dal titolo, uno stato di disponibilità particolare, …); uno scraper a cui non serve non chiama nulla e la lista resta vuota. Il core non interpreta i tag: li persiste e la UI li mostra (visione a lungo termine: tag grafici). Le regole *site-specific* (quali etichette esistono, come riconoscerle) restano nel plugin concreto, mai nella base.
