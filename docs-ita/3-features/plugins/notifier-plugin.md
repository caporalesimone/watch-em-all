...
- **NOT-R4** — Il plugin riceve la **lingua dell'utente** e genera i testi in quella lingua con le **proprie traduzioni di backend** (cartella `backend/i18n/` del plugin, lato server — distinta dalla `frontend/i18n/` della sua UI). **`en.json` è obbligatorio e sempre completo**: se la lingua richiesta manca nel plugin, il fallback è `en`. **V1 English-only**: arriva sempre `en` e il plugin spedisce solo `en.json` — ma le stringhe stanno comunque dietro chiavi, mai cablate. La valuta è resa come simbolo (default €).
...
- **NOT-R6** — **Test**: ogni notifier implementa l'invio di una **notifica di prova** con la config corrente (merge admin+utente). **Solo admin da 10.X4**: la prova risponde a *"la configurazione di sistema funziona?"*, che è una domanda di chi quella configurazione la può correggere; il recapito è l'account dell'admin stesso, non un indirizzo scritto in un campo.
- **NOT-R9** — **Un canale si valida prima di poter essere acceso** (10.B28). La prova di NOT-R6 *è* la validazione: `POST /api/admin/notifiers/{id}/validate` manda un messaggio vero e, **se il server lo prende in carico**, registra quelle impostazioni come validate. Finché non lo sono, l'interruttore rifiuta di accendersi (`422 not_validated`) e il canale non è disponibile per nessuno. Tre conseguenze da dire esplicitamente:
  - **Quel che si afferma è poco, ed è voluto**: il server ha accettato il messaggio. Se poi lo consegni, lo metta in spam o lo rimbalzi è affare fra quel server e il destinatario — un portale che pretendesse di saperlo dovrebbe diventare un monitor di posta.
  - **La prova riguarda una configurazione, non un canale**: quel che si memorizza è l'impronta delle impostazioni che hanno funzionato, quindi modificarne una invalida la prova **per aritmetica** e non perché qualcuno si ricorda di azzerare un flag — e un canale le cui impostazioni cambiano viene spento.
  - **Un fallimento non registra niente**, così nessun canale scivola in "validato" per il solo fatto di essere stato provato.
  L'in-app è esente: non ha un server che possa accettare qualcosa né una config da sbagliare.
