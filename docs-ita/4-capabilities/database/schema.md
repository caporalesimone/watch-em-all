...
| `scraper_admin_config` | plugin_id PK, config_json, updated_at | config admin dello scraper (PCFG, 4.B10): `config_json` contiene i **campi dichiarati dal plugin** (regole del sito, fase 7+) **e** le **chiavi riservate del core** (`politeness_delay_s`, `http_timeout_s`, `cache_ttl_min`, `scrape_now_min_interval_s`) lette dal core (client HTTP, cache, cooldown dello scrape-now) e non dal plugin (CTX); default = le costanti che sostituiscono, override merge-ati, chiavi ignote ignorate. **Niente `enabled`**: la sospensione è in `scraper_schedule`. Speculare a `notifier_admin_config` (che invece ha `enabled` = interruttore del canale) |
