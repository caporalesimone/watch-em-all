...
- **BE-13** — `datetime.now(tz=UTC)` o l'orologio applicativo iniettabile; mai `utcnow()` naïve. I timestamp persistiti sono **UTC**; gli orari inseriti (slot, alert) sono interpretati nel **fuso configurato** (`TZ`, default `Europe/Rome`) con conversioni esplicite via `zoneinfo` — mai affidarsi all'ora locale implicita del processo. Un solo fuso per installazione.
