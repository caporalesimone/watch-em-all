...
- **PLG-7** — `external_id`: il plugin implementa **solo** il seme astratto `identity_seed` (SCR-R10); l'hashing/normalizzazione è imposto dalla base (`final`) e **mai reimplementato**. La scelta del seme è documentata nella doc del plugin in `implemented-plugins/`; cambiarla è un breaking change per i dati degli utenti e si fa solo con una nota di migrazione. Mai riempire `external_id` a mano né hashare con `hash()` built-in.
