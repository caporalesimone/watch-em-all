...
- **FE-18** — I widget **candidati alla condivisione** tra plugin (es. **Scrape-now**, tabella **dry-run** [SCR-R12](../../3-features/plugins/scraper-plugin.md), **form di config dinamico** 7.F1, bottoni Test/Send, popup/overlay, chip di stato) si scrivono **self-contained e props-driven**, senza coupling al singolo plugin, così che l'estrazione in `$lib/components` al **secondo uso** (FE-8) sia un semplice spostamento e non un refactor. Le loro stringhe i18n, una volta estratti nel core, vivono in un **namespace core condiviso** (es. `ui.*`), non duplicate per-plugin (FE-13/FE-17). Finché c'è **un solo** consumatore l'astrazione resta prematura: si tiene il componente locale ma già scollegato, pronto al lift.
