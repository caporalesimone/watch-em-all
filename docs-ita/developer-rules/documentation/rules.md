...
- **DOC-13** — **Stile del `CHANGELOG.md`** (deciso 2026-06-20): ogni voce è **breve** e leggibile dall'utente come **storia di cosa è cambiato per lui**. Due parti: (1) **bullet point** sui cambiamenti di **esperienza utente** — aggiunte, rimozioni e modifiche **insieme**, frasi brevi, gergo tecnico al minimo; (2) un **breve paragrafo discorsivo** (`_Under the hood:_`) sui cambiamenti **architetturali/tecnici**, con dettaglio tecnico essenziale. Niente sezioni `Added/Changed/Fixed` prolisse; la voce **non deve essere lunga**. Le voci storiche restano come sono; la regola vale d'ora in poi. Il requisito **1 PR = 1 voce + bump** resta (INF-19).
