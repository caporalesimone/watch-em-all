...
- **Digest** (`alert_digest`): a concise subject; an HTML body with one section per cart — event
  badges, a product table with provenance (the source plugin id), before → after price, discount and
  link; totals and threshold. A `text/plain` fallback is always included.
