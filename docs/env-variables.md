...
| `WEA_SCHEMA_DRIFT_ALERT` | web | _unset → **off**_ | Dev safety net (4.B0). The startup schema-drift check **always runs and logs warnings**; this flag only controls whether the drift appears in the **admin-only** errors feed `GET /api/admin/errors` (and thus the red admin banner) — never on the public `/api/health`. `.env`/`.env.example` ship it `true`. |
