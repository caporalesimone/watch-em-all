...
| `WEA_SCHEMA_DRIFT_ALERT` | web | _unset → **off**_ | Dev safety net (4.B0). The startup schema-drift check **always runs and logs warnings**; this flag only controls whether the **admin-only** endpoint `GET /api/admin/schema-drift` exposes the drift (and thus whether the red admin banner shows) — never on the public `/api/health`. `.env`/`.env.example` ship it `true`. |
