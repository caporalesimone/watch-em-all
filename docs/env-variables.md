...
| `WEA_SCHEMA_DRIFT_ALERT` | web | _unset → **off**_ | Dev safety net (4.B0). The startup schema-drift check **always runs**, always logs, and — since 0.9.0 — always serves the [incompatibility page](4-capabilities/core/schema-compatibility.md) while drift lasts (INC-R1): none of that is optional, because it is what keeps a mismatched database from being written to. This flag only controls whether the drift also appears in the **admin-only** errors feed `GET /api/admin/errors` (and thus the red admin banner) — never on the public `/api/health`. `.env`/`.env.example` ship it `true`. |
