...
9. **Ogni PR = una versione, ogni fase = una release**: ogni PR porta un **bump di versione** (SemVer) e una voce in **`CHANGELOG.md`**; senza, non è mergiabile (INF-19). I tag **non** sono per-PR: il tag `vX.Y.Z` nasce **automaticamente alla chiusura di una fase** (13 fasi = 13 tag, [ci](../infrastructure/ci.md#tag-di-fine-fase-automatico)); le versioni intermedie vivono solo nel CHANGELOG.
