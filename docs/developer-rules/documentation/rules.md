...
- **DOC-4** — Lingua italiana; identificatori, termini tecnici e nomi di campo in inglese. Link sempre relativi.
...
- **DOC-12** — **Documentazione inglese incrementale (`docs-eng/`)**: alla **chiusura di ogni fase** del [development flow](../../development-flow/README.md), la cartella `docs-eng/` (root del repo, accanto a `docs/`) si aggiorna con l'**equivalente inglese** della documentazione di `docs/` — **solo ed esclusivamente la parte implementata** in quella fase, su tutti i livelli toccati (layer 1-4 e sezioni trasversali), con la **stessa alberatura** di `docs/`. La documentazione inglese cresce con il sito: mai descrivere in inglese ciò che non è ancora implementato, mai lasciare indietro ciò che lo è. È un item della Definition of Done di ogni fase.
