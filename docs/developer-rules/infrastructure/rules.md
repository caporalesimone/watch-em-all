...
- **INF-19** — **Versioning del prodotto**: **SemVer** (`MAJOR.MINOR.PATCH`), **versione unica per il bundle** (core + plugin first-party). Il tag `vX.Y.Z` guida immagini e deploy kit (INF-17) ed è **deliberato**, non a ogni merge; **MAJOR** su breaking dell'API pubblica o schema DB non additivo (DB-R4); `CHANGELOG.md` aggiornato nella PR del tag. Distinta dall'`api_version` dei plugin (gate di contratto) e dal `version` informativo del manifest. Dettaglio: [ci](../../infrastructure/ci.md#versioning-del-prodotto).
