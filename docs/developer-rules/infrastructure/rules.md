...
- **INF-19** — **Versioning del prodotto**: **SemVer** (`MAJOR.MINOR.PATCH`), **versione unica per il bundle** (core + plugin first-party). **Ogni PR porta un bump di versione e una voce `CHANGELOG.md`** (senza, non è mergiabile); **al merge** l'owner crea il tag `vX.Y.Z` corrispondente, che genera l'immagine di release e alimenta il deploy kit (INF-17) — **1 MVP = 1 PR = 1 versione**. **MAJOR** su breaking dell'API pubblica o schema DB non additivo (DB-R4). Le immagini **dev** di anteprima sono taggate `dev-<branch>` (mutabili, su PR; vedi [ci](../../infrastructure/ci.md#immagini-dev-su-pr)). Distinta dall'`api_version` dei plugin (gate di contratto) e dal `version` informativo del manifest. Dettaglio: [ci](../../infrastructure/ci.md#versioning-del-prodotto).
