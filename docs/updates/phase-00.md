...
- **Publish on tag**: pushing an `x.y.z` tag builds and pushes the versioned images. The **deploy kit** (`compose.yml` + `.env.example`) lives in the repo and is fetched at the tag — not attached to the release.
...
docker compose -f compose-dev.yml down -v    # stop + wipe the database volume
```
