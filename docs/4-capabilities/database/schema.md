...
| Table | Columns | Notes |
|---|---|---|
| `products` | id, user_id FK **CASCADE**, plugin_id, external_id, url, name, image_url, brand_text, brand_link, tags (JSON), category (JSON), extra_json (JSON), currency, price_current, price_original, discount_pct, is_available, removed, **removed_at**, first_seen_at, last_seen_at, **observations, cache_hits, price_changes, availability_changes, price_min, price_min_at, price_max, price_max_at, last_price_change_at** | **UNIQUE (user_id, plugin_id, external_id)** = product identity; per-user catalog. `brand_text`/`brand_link` (PROD-R6, optional link), `tags` (JSON array of strings, PROD-R5) and `category` (JSON array of `{text, link}`, breadcrumb, PROD-R7) are scraper data, persisted without interpretation. `removed_at` = when the delisting sweep dropped it (NULL whenever `removed` is false): `removed` alone cannot say *since when*. The counters after it are the **per-product statistics** (9.B6b), written by the catalog service, counted per catalog row and therefore per user — `observations` counts **fresh reads only** (a delivery served from the scrape cache increments `cache_hits` instead, and for a category product one HTTP cache hit serves up to 50 products, so it means "my data came from a cached page", not "one request saved for me"). `price_changes`/`availability_changes` here are **not** `scrape_run.price_changes`, which also counts the first history row and availability moves |
| `process_status` | process (PK), last_seen_at, state, detail | What each process of the installation reports about itself ([process-status](../core/process-status.md), PST-R1): the web and the worker share nothing but the database. **One row per process, updated in place** — a heartbeat is a state, not an event, and appended it would be ~525.000 rows a year to answer a question that reads the latest one. The writer rate-limits itself to one beat every 30 s (PST-R2) |
| `plugin_jobs` | id, plugin_id, job_key, progress_done, progress_total, detail, cancel_requested, updated_at | Progress and cooperative cancellation of one in-flight plugin job (CTX-R13, C9/C10). Written by the **core** on its own short-lived session, because publishing progress means committing while the work runs — and the session a plugin holds during a scheduled run is the worker's. `job_key` is the plugin's own; **UNIQUE (plugin_id, job_key)**. A finished job keeps its row (its last progress is what the page shows afterwards), so `begin` clears a stale cancellation |
| `product_sources` | id, product_id FK **CASCADE**, source_key, source_kind, source_label | Which of the user's inputs deliver a product (C14) — **many-to-many**: the same product can arrive from two categories, which is why it never got a single FK to a watch. The source is *described* rather than joined (a watch lives in the plugin's schema, CTX-R6): `source_key` is the plugin's own id for that input, `source_label` a name to show, refreshed on every delivery. **UNIQUE (product_id, source_key)**; dropped by `forget_source` when the plugin says the input is gone |
| `price_history` | id, plugin_id, external_id, price_current, price_original, discount_pct, is_available, recorded_at | Append-only, **per product and not per user**: keyed on the identity `(plugin_id, external_id)`, deliberately **no FK** into `products` (per-user, and its cascade used to delete the history of a removed product). One entry **only** on a price **or** availability change, whoever was delivered it; INDEX (plugin_id, external_id, recorded_at); **no retention**, and nothing cascades into it |
...
| `scrape_run` | run_id, scraper_id, trigger, slot, started_at, finished_at, status, users_processed, products_found, products_new, price_changes, products_removed, products_excluded, http_requests, cache_hits, error_message | one row per run; INDEX (scraper_id, started_at); retention. `products_excluded` = what the **scraper** filtered out before delivering (Dragon Store's dented listings): it comes from `DeltaCounters.excluded`, which only a plugin can fill — the catalog service is handed the survivors |
| `scraper_stats` | plugin_id PK, since, runs_total, runs_ok, runs_failed, runs_skipped_locked, last_run_at, last_success_at, last_failure_at, consecutive_failures, http_requests_total, cache_hits_total, bytes_downloaded_total, politeness_wait_s_total, run_seconds_total, rate_limited_total, gate_hits_total, gate_cleared_total, robots_denied_total, products_delivered_total, pages_fetched_total, parse_failures_total | **lifetime** statistics per scraper (9.B6c), **global** (not per user) and cumulative: `scrape_run` has retention, so aggregating it answers "recently", never "ever". The health group (rate limits, anti-bot gate, `robots.txt` refusals) is information nothing else records — it lived only as log lines, which is exactly what was missing during the 25 July block, when the question was "since when, and how often". `since` is part of the contract, not decoration: a counter that never resets misleads after a configuration change (politeness went from 1.5 s to 11 s in 0.8.1, and totals either side are not comparable). How they are shown is a later phase; 0.9.0 only collects them |
...
- **DB-R1** — Every operational table carries `user_id`; every application query filters by the token's user (multi-tenancy). `products` is scoped this way. **`price_history` is the one deliberate exception**: a price is a fact about the site rather than about a user, so it is keyed on the product's identity, carries no `user_id`, and is reached only *through* a row the caller owns — the router resolves the identity of a product the user owns and reads the shared chain with it.
