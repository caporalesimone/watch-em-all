...
- **DB-R5** — Backup/export/ripristino: script versionati in `ops/`, montati nel container `db` ed eseguiti a mano; il dump include tutte le configurazioni (config DB-first) e l'archivio anche i file di bootstrap ([backup-and-restore](../../infrastructure/backup-and-restore.md)).
