...
- Il tag si crea **da CLI** (`git tag x.y.z && git push origin x.y.z`) **o dalla UI di GitHub** pubblicando una release (GitHub non sa creare tag "nudi": il tag nasce con la release). Il job di release è **idempotente** — se la release esiste già (creata da UI) allega solo il deploy kit, altrimenti la crea lui — quindi entrambe le vie funzionano.
