...
- **SCHED-R8** — **Per-scraper politeness**: minimum delay between consecutive HTTP requests of the same scraper — reserved key **`politeness_delay_ms`** (milliseconds, default 1000–2000 ms), configurable per scraper on its admin page (4.B10). It is enforced by the HTTP client provided by the core, not left to the plugin's goodwill. **Upwards only** (10.B22): a `Crawl-delay` published by the site is a hard minimum, and a smaller configured value is raised to it rather than applied. The number of retries on a failed request is the reserved key `http_retries`, configurable beside it since 10.B22 — each retry is another request to a site that has already not answered, so its ceiling is deliberately low.
