...
### Building a Product
- **SCR-R18** — The base **assembles** the `Product` (`build_product`, `final`): the plugin passes what the *site* said, the base imposes the parts of the contract that are identical for every scraper. Those parts are `external_id` always through the identity template-method (SCR-R10), `discount_pct` always `None` because the core derives it (CATSVC-R3), `scraped_at` = the caller's `fetched_at` and the clock only when there is none (PROD-R8), `extra` filtered on `None` and **only** on `None`, and `availability` read as the **schema.org** vocabulary (`InStock`/`OutOfStock`/`PreOrder`, the last one orderable and tagged `Pre Order`) — a web standard, not one site's invention, so an unknown token is logged rather than guessed at. What stays the plugin's is the site knowledge: URL grammar, pagination, how to read a price the site does not print, which labels its sanitiser strips (`tags` therefore arrive already accumulated, SCR-R16). This is a contract *enforced* rather than documented: the same assembly written by hand in three places across two plugins had already drifted — a discarded `fetched_at`, and two different predicates for filtering `extra`. It also shortens what a new scraper's author has to know: they describe a site, not the core's expectations.
