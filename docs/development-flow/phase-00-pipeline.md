...
- [x] **0.T5 — Compose di sviluppo** (~1h): `docker-compose.yml` con `db`+`web`+`worker` (+`adminer` profilo `dev`, `ops` profilo `ops`), healthcheck e log rotation (INF-2) ([deployment](../infrastructure/deployment.md)). *Verifica: `compose up` → tutti healthy, segnaposto nel browser.*
- [x] **0.T6 — CI di base su PR** (~1h): workflow GitHub Actions che builda le tre immagini a ogni PR + **guardia CHANGELOG** (PR rossa se `CHANGELOG.md` non è aggiornato, INF-19) ([ci](../infrastructure/ci.md)). I linter arrivano col codice (1.T1). *Verifica: PR senza voce CHANGELOG → rossa; con voce → verde.*
- [x] **0.T7 — Immagini dev sul branch** (~1h): job che pubblica `web`/`worker`/`ops` su GHCR con tag **`dev-<branch>`** (mutabile, su PR e `workflow_dispatch`) ([ci — immagini dev](../infrastructure/ci.md#immagini-dev-su-pr)). *Verifica: push sul branch → `docker pull` anonimo di `dev-<branch>` funziona.*
...
- [x] **0.T9 — Publish su tag + deploy kit** (~1h): workflow su tag `x.y.z` → push delle tre immagini versionate su GHCR + release GitHub con `deploy/compose.yml` (immagini, niente `build:`, mount di override `config.yaml` commentato) e `.env.example` con `WEA_VERSION` allegati ([ci — publish](../infrastructure/ci.md), INF-17). *Verifica: tag → release con i due file, immagini taggate `x.y.z`.*
