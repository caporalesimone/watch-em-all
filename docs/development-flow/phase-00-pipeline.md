...
- [x] **0.T2 — Dev container** (~1h): `.devcontainer/` (Dockerfile con Python 3.12+Poetry, Node 22+npm, git, docker CLI, `gh`; socket Docker dell'host; forward 8080/8081; post-create tollerante) ([dev-container](../infrastructure/dev-container.md), INF-15). *Verifica: "Reopen in Container" su host con solo Docker → poetry/npm/compose/gh funzionano da dentro.*
